...
def maf_to_csv(maf_file, score_threshold=None, csv_outpath=None):
    ...
    maf_dict = {'score':[], 'EG2':[], 'E':[], 'chromosome':[],
                'nuc_start':[], 'nuc_strand':[], 'nuc_srcsize':[], 'nuc_size':[],
                'mtc_start':[], 'mtc_strand':[], 'mtc_srcsize':[], 'mtc_size':[],
                'nuc_seq':[], 'mtc_seq':[]}
...
def merge_maf_and_blasttab(mafpath, btpath, outfile = None):
    ...
    columns = ['query id', 'subject_id', '%_identity', 'alignment_length', 'mismatches', 'gap_opens', 'query_start', 'query_end', 'subject_start', 'subject_end', 'evalue', 'bit_score']
    ...
    if consider_strands==True:
        if len(region_df['mtc_strand'].unique())>1:
            for index,row in region_df.iterrows():
                if row['mtc_strand']==1:
                    g=sns.lineplot(x=[row['nuc_start'],row['nuc_end']], y=[row['mtc_start'],row['mtc_end']], linewidth = 3, color='blue')#, label=str(row['numt_id']))
                else:
                    g=sns.lineplot(x=[row['nuc_start'],row['nuc_end']], y=[row['mtc_end'],row['mtc_start']], linewidth = 3, color='blue')#, label=str(row['numt_id']))
        else:
            for index,row in region_df.iterrows():
                g=sns.lineplot(x=[row['nuc_start'],row['nuc_end']], y=[row['mtc_start'],row['mtc_end']], linewidth = 3, color='blue')#, label=str(row['numt_id']))
    else:
...
def plot_region_with_identity(region_df, save_path=None, consider_strands=True):
    ...
    if consider_strands==True:
        if len(region_df['mtc_strand'].unique())>1:
            for index,row in region_df.iterrows():
                if row['mtc_strand']==1:
                    g=sns.lineplot(x=[row['nuc_start'],row['nuc_end']], y=[row['mtc_start'],row['mtc_end']], linewidth = 3, color='blue', label=str(round(row['%_identity'],0)))
                else:
                    g=sns.lineplot(x=[row['nuc_start'],row['nuc_end']], y=[row['mtc_end'],row['mtc_start']], linewidth = 3, color='blue', label=str(round(row['%_identity'],0)))
        else:
            for index,row in region_df.iterrows():
                g=sns.lineplot(x=[row['nuc_start'],row['nuc_end']], y=[row['mtc_start'],row['mtc_end']], linewidth = 3, color='blue',  label=str(round(row['%_identity'],0)))
    else:
...
def assign_range_and_plot(df, range, outcol, column='nuc_size', outpath=None, x_axis_label=None, y_axis_label=None):
    ...
    ax = sns.countplot(data=df, x=outcol, order=rangeorder, color=sns.color_palette()[0])
