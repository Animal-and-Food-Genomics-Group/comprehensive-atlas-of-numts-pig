...
# main_numts_df=pd.read_csv("results/numt_discovery/numts/Sus_scrofa/Sscrofa10.2/GCF_000003025.5_Sscrofa10.2_numts.csv")
# other_numts_df=pd.read_csv("results/numt_discovery/numts/Sus_scrofa/Sscrofa11.1/GCF_000003025.6_Sscrofa11.1_numts.csv")
...
# df=maf_to_csv("data/genome_comparison/regions_extracted_from_maf/Sus_scrofa/Sscrofa10.2/GCF_000003025.6_in_GCF_000003025.5.maf")
# df_blast=pd.read_table("data/genome_comparison/regions_extracted_from_maf/Sus_scrofa/Sscrofa10.2/GCF_000003025.6_in_GCF_000003025.5.blasttab", header=None)
...
df_main_bed = BedTool.from_dataframe(
    df[["main_chromosome", "main_start", "main_end", "index"]]
)
df_other_bed = BedTool.from_dataframe(
    df[["other_chromosome", "other_start_forward", "other_end_forward", "index"]]
)
main_numts = pd.read_table(snakemake.input.main_regions, header=None)
# main_numts=pd.read_table("data/numt_discovery/region_sequences/Sus_scrofa/Sscrofa10.2/GCF_000003025.5_Sscrofa10.2.bed", header=None)
main_numts_bed = BedTool.from_dataframe(main_numts)
other_numts = pd.read_table(snakemake.input.other_regions, header=None)
# other_numts=pd.read_table("data/numt_discovery/region_sequences/Sus_scrofa/Sscrofa11.1/GCF_000003025.6_Sscrofa11.1.bed", header=None)
...
df["main_size/other_size"] = df["main_size"] / df["other_size"]
...
for name, group in df.groupby(by="other_region_id"):
    if len(group["main_region_id"].unique()) == 1:
        corr_dict[name] = group["main_region_id"].iloc[0]
    else:
...
for i in multimatches:
    biggest = None
    biggest_sum = 0
    for name, group in i.groupby(by="main_region_id"):
        if group["aln_length"].sum() > biggest_sum:
            biggest = name
            biggest_sum = group["aln_length"].sum()
    corr_dict[i["other_region_id"].iloc[0]] = biggest

newdf = []

for k, v in corr_dict.items():
    newdf.append(df[(df["other_region_id"] == k) & (df["main_region_id"] == v)])
...
for name, group in newdf.groupby(by="main_region_id"):
    if name != ".":
        if len(group["other_region_id"].unique()) > 1:
            ...
            for name2, group2 in group.groupby(by="other_region_id"):
        ...
        for name2, group2 in group.groupby(by="other_region_id"):
            ...
            group2 = group2.drop_duplicates(subset="other_region_id", keep="first")
            newdf = pd.concat([newdf, group2])

newdf["other_start_forward"] = newdf["other_start"].where(
    newdf["other_strand"] == "+", newdf["other_srcsize"] - newdf["other_end"]
)
newdf["other_end_forward"] = newdf["other_end"].where(
    newdf["other_strand"] == "+", newdf["other_srcsize"] - newdf["other_start"]
)
...
outdf = pd.DataFrame(
    columns=[
        ...
        "main_region_end",
        "alignment_length",
        "alignment_length_by_main_region_length",
        "alignment_length_by_other_region_length",
        "gaps_by_alignment_length",
    ]
)
...
for name, group in newdf.groupby(by="other_region_id"):
    outdf.loc[len(outdf)] = (
        name,
        group["main_region_id"].unique()[0],
        group["main_chromosome"].unique()[0],
        group["main_start"].astype(int).min(),
        group["main_end"].astype(int).max(),
        group["aln_length"].astype(int).sum(),
        group["aln_length/main_region_size"].sum().round(2),
        group["aln_length/other_region_size"].sum().round(2),
        group["main_gaps/aln_length"].sum().round(2),
    )


status = []
for index, row in outdf.iterrows():
    if (row["gaps_by_alignment_length"] > 0.25) & (row["main_region_id"] == "."):
        tempstatus = "PRIVATE"
    elif (row["main_region_id"] == ".") & (
        row["alignment_length_by_other_region_length"] > 0.8
    ):
        ...
        tempstatus = "DROP"
    elif (row["alignment_length_by_other_region_length"] < 0.5) | (
        row["alignment_length_by_main_region_length"] < 0.5
    ):
        tempstatus = "DROP"
    else:
