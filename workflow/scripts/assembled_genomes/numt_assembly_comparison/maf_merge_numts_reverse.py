...
outdf=pd.DataFrame(columns=['other_region_id','main_region_id','main_chromosome','main_region_start','main_region_end','alignment_length','alignment_length_by_main_region_length','alignment_length_by_other_region_length','gaps_by_alignment_length'])


for name,group in newdf.groupby(by='other_region_id'):
    outdf.loc[len(outdf)]=(name,group['main_region_id'].unique()[0], group['main_chromosome'].unique()[0],
    group['main_start'].astype(int).min(), group['main_end'].astype(int).max(),
    group['aln_length'].astype(int).sum(), group['aln_length/main_region_size'].sum().round(2), group['aln_length/other_region_size'].sum().round(2), group['main_gaps/aln_length'].sum().round(2)
    )
...
for index,row in outdf.iterrows():
    if (row['gaps_by_alignment_length']>0.25)&(row['main_region_id']=='.'):
        tempstatus='PRIVATE'
    elif (row['main_region_id']=='.')&(row['alignment_length_by_other_region_length']>0.8):
