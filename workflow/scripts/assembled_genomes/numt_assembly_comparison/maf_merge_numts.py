...
for name, group in newdf.groupby(by="main_region_id"):
    outdf.loc[len(outdf)] = (
        name,
        group["other_region_id"].unique()[0],
        group["other_chromosome"].unique()[0],
        group["other_start_forward"].astype(int).min(),
        group["other_end_forward"].astype(int).max(),
        ...
        group["other_gaps/aln_length"].sum().round(2),
    )
...
for index, row in outdf.iterrows():
    if (row["gaps_by_alignment_length"] > 0.25) & (row["other_region_id"] == "."):
        tempstatus = "PRIVATE"
    elif (row["other_region_id"] == ".") & (
        row["alignment_length_by_main_region_length"] > 0.8
    ):
