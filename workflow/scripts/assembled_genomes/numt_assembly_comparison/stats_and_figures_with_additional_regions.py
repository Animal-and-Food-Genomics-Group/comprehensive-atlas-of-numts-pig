...
for name, group in df.groupby(by="genome"):
    ...
    custom_functions.assign_range_and_plot(
        assembly,
        range=size_ranges,
        outcol=f"NUMT size range in {assembly_name} genome (bp)",
        y_axis_label="Count",
        outpath=f"{snakemake.params[0]}/{name}/numt_size_distribution_assembly.png",
    )
    custom_functions.assign_range_and_plot(
        assembly,
        range=identity_ranges,
        outcol=f"NUMT sequence identity range in {assembly_name} genome",
        outpath=f"{snakemake.params[0]}/{name}/numt_identity_distribution_assembly.png",
    )
...
plt.savefig(
    f"{snakemake.params[1]}/size_distribution.png", dpi=300, bbox_inches="tight"
)
...
plt.savefig(
    f"{snakemake.params[1]}/identity_distribution.png", dpi=300, bbox_inches="tight"
)
