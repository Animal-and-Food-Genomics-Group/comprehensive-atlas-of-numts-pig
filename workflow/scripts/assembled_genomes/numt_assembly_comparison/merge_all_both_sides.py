...
for index,row in genomes.iloc[1:-2].iterrows():
    ...
    tempdf=pd.read_csv(f"test/genome_comparison/regions_extracted_from_maf/Sscrofa11.1/{other_species}/{other_assembly_name}/both_sides_compared/GCF_000003025.6_vs_{other_assembly_accession}.csv")
    tempdf['other_status']=tempdf['main_region_status']+"|"+tempdf['other_region_status']
    tempdf=tempdf.drop(columns=['main_region_status','other_region_status'])
    for i in ['main_region_start','main_region_end','other_region_start','other_region_end']:
        tempdf[i]=tempdf[i].astype(int)
    tempdf['main_region_size']=tempdf['main_region_end']-tempdf['main_region_start']
    tempdf['other_region_size']=tempdf['other_region_end']-tempdf['other_region_start']
    ...
    tempdf.columns=[i.replace("other",row['Genome Abbreviation']) for i in tempdf.columns]
...
otherdf=otherdf.sort_values(by=['REF11_chromosome','REF11_region_start','REF11_region_end'])

otherdf_bed=BedTool.from_dataframe(otherdf[['REF11_chromosome','REF11_region_start','REF11_region_end']])
...
chromdict={i:j for i,j in zip(report['RefSeq-Accn'], report['Chromosome/Scaffold Name'])}
...
newcolumns=['region_id','REF11_chromosome_name']+[i for i in outdf.columns if ("REF11" in i)&("chromosome_name" not in i)]
...
outdf['REF11_region_id']=(outdf['REF11_chromosome'].astype(str)+":"+outdf['REF11_region_start'].astype(str)+"-"+outdf['REF11_region_end'].astype(str)).where(outdf['REF11_status'].str.contains("COMPATIBLE_SEQUENCE\|"), outdf['REF11_region_id'])
...
for i in genomes[1::]:
    outdf[f'{i}_region_id']=(outdf[f'{i}_chromosome'].astype(str)+":"+outdf[f'{i}_region_start'].fillna(0).astype(int).astype(str)+"-"+outdf[f'{i}_region_end'].fillna(0).astype(int).astype(str)).where(outdf[f'{i}_status'].str.contains("\|COMPATIBLE_SEQUENCE"), outdf[f'{i}_region_id'])
