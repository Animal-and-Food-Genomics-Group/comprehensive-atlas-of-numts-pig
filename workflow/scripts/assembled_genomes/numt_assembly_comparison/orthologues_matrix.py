...
dfs['region_id']=dfs['region_id'].where(dfs['additional_region_id'].isna(), dfs['additional_region_id'])

single_genome_regions={i:0 for i in genomes['Genome Abbreviation']}

for i in single_genome_regions.keys():
    tempdf=dfs[dfs['genome']==i]
    single_genome_regions[i]=len(tempdf[~tempdf['region_id'].isin(df[f'{i}_region_id'])]['region_id'].unique())
...
for a in genomes['Genome Abbreviation']:
    ...
    for b in genomes['Genome Abbreviation']:
        if a!=b:
            ...
            not_found_matrix[a][b]=len(dfs.loc[a]['region_id'].dropna().unique())-found_matrix[a][b]
        else:
...
not_found_df=not_found_df[genomes['Genome Abbreviation']].loc[genomes['Genome Abbreviation']]

found_matrix_lower = np.tril(found_df.astype(str))
not_found_matrix_upper=np.triu(not_found_df.transpose().astype(str)+"|")+np.triu(not_found_df.astype(str))
...
for i in single_genome_regions.keys():
    matrix.loc[i][i]=str(len(dfs.loc[i]['region_id'].unique()))+"|"+str(single_genome_regions[i])
