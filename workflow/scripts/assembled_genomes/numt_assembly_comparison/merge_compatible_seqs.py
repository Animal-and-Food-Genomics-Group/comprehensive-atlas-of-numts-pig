...
if len(df)>0:
    ...
    df['seq_start']=df['chromosome'].str.split(":").str[1].str.split("-").str[0].astype(int)
    df['seq_end']=df['chromosome'].str.split(":").str[1].str.split("-").str[1].astype(int)
    ...
    df['chromosome_type']=df['chromosome_code'].apply(lambda x: scaffold_or_chromosome_dict[x])


    df=df.sort_values(by=['chromosome_type','chromosome','nuc_start'], key=natsort_keygen()).reset_index(drop=True)

    chromsizes={i:j for i,j in zip(report['Sequence-ID'], report['Sequence-Length'])}
    ...
    for name,group in ref.groupby(by='chromosome'):
        max_region_dict[name]=group['region_id'].str.split("_").str[-1].astype(int).max()
    ...
    for name,group in df_bed.groupby(by='chromosome'):
        ...
        regions_bed['region_id']=regions_bed['chrom']+"_"+"REGION"+"_"+(regions_bed['region_id']+max_region_dict[name]).astype(str)
    ...
    significantly_overlapping_regions=df[df['region_id'].isin(overlapping_numts_df[overlapping_numts_df['distance_to_next_numt_nuc']/overlapping_numts_df['nuc_size']<=-0.5]['region_id'])]
    ...
    for name,group in significantly_overlapping_regions.groupby(by='region_id'):
        ...
        for i,r in group[group['numt_id'].isin(list(numts_to_keep))].iterrows():
            if len(set(range(check_for_last_overlap['nuc_start'], check_for_last_overlap['nuc_end']+1)).intersection(set(range(r['nuc_start'], r['nuc_end']+1))))>=check_for_last_overlap['nuc_size']/2:
                numt_to_keep=None
            else:
                continue
    ...
    numts_to_drop_from_overlap_regions_df = significantly_overlapping_regions[~significantly_overlapping_regions['numt_id'].isin(all_numts_to_keep)]
    ...
    not_found.columns=['chromosome_code','nuc_start','nuc_end','additional_region_id']
    if len(not_found)>0:
        ...
        not_found['chromosome']=not_found['chromosome_code'].apply(lambda x:renaming_dict[x])
        ...
        not_found['chromosome_type']=not_found['chromosome_code'].apply(lambda x:scaffold_or_chromosome_dict[x])
        ...
        not_found['numt_id']=not_found.reset_index()['index'].apply(lambda x: numt_ids_dict[x])
