...
private = non_corr[non_corr["status"] == "PRIVATE"].sort_values(
    by="alignment_length", ascending=False
)
...
corr = corr.sort_values(
    by=["alignment_length", "main_chromosome", "other_chromosome"], ascending=False
)
...
for index, row in corr.iterrows():
    tempmain = main_numts_df[main_numts_df["region_id"] == row["main_region_id"]].iloc[
        0
    ]
    tempother = other_numts_df[
        other_numts_df["region_id"] == row["other_region_id"]
    ].iloc[0]
    outdf.loc[len(outdf)] = [
        tempmain["region_id"],
        tempmain["chromosome_code"],
        tempmain["region_start"],
        tempmain["region_end"],
        "CORRESPONDING",
        tempother["region_id"],
        tempother["chromosome_code"],
        tempother["region_start"],
        tempother["region_end"],
        "CORRESPONDING",
    ]
...
for index, row in comp_main.iterrows():
    ...
    outdf.loc[len(outdf)] = [
        ...
        "CORRESPONDING",
        ".",
        row["other_chromosome"],
        row["other_region_start"],
        row["other_region_end"],
        "COMPATIBLE_SEQUENCE",
    ]
