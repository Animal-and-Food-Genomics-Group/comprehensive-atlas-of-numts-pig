...
all['REF11_strand']='+'
all_bed=all[['REF11_chromosome','REF11_region_start','REF11_region_end','REF11_strand','REF11_region_id']]
...
df_bed=BedTool.from_dataframe(df[['REF11_chromosome','REF11_start','REF11_end','REF11_strand','genome']])

df_merged=df_bed.intersect(all_bed, wao=True).to_dataframe()

revdf_bed=BedTool.from_dataframe(revdf[['REF11_chromosome','REF11_start','REF11_end','REF11_strand','genome']])
...
df_merged.columns=['REF11_chromosome','REF11_start','REF11_end','genome','REF11_region_id','overlap']
df=df_merged.merge(df, on=list(df_merged.columns[0:4]))

revdf_merged=revdf_merged[revdf_merged['blockCount']!='.']
revdf_merged=revdf_merged[['chrom','start','end','score','blockCount','blockSizes']]
revdf_merged.columns=['REF11_chromosome','REF11_start','REF11_end','genome','REF11_region_id','overlap']
...
for index,row in all.iterrows():
    newrow=[row['region_id'],row['REF11_chromosome_name'],row['REF11_region_id'],row['REF11_chromosome'],row['REF11_region_start'],row['REF11_region_end'],row['REF11_region_size'], row['REF11_status']]
    ...
    for genome in genome_names:
        if row[f'{genome}_region_id']!=row[f'{genome}_region_id']:
            if len(tempdf[tempdf['genome']==genome])>0:
                ...
                for i in [newid, genome_df['other_chromosome'],genome_df['other_start_forward'], genome_df['other_end_forward'], 'COMPATIBLE_SEQUENCE|COMPATIBLE_SEQUENCE', (genome_df['other_end']-genome_df['other_start'])]:
                    newrow.append(i)
            else:
                for i in range(0,6):
                    newrow.append(np.nan)
        else:
            for i in [row[f'{genome}_region_id'], row[f'{genome}_chromosome'], row[f'{genome}_region_start'], row[f'{genome}_region_end'], row[f'{genome}_status'], row[f'{genome}_region_size'] ]:
                newrow.append(i)
