...
for name,group in df_bed.groupby(by='chromosome'):
    ...
    regions_bed['region_id']=regions_bed['chrom']+"_"+"REGION"+"_"+regions_bed['region_id'].astype(str)
...
numts_too_close_to_extremity=df[(df['region_start']<1000)|((df['nuc_srcsize']-df['region_end'])<1000)]
numts_too_close_to_extremity.to_csv(snakemake.output.dropped_putative_numts_close_to_extremity)
...
Path(snakemake.output.duplicate_numts_plots_directory).mkdir(parents=True, exist_ok=True)

for name,group in significantly_overlapping_regions.groupby(by='region_id'):
    custom_functions.plot_region(group, save_path=os.path.join(snakemake.output.duplicate_numts_plots_directory, name+".png"))
...
numts_to_drop_from_overlap_regions_df.to_csv(snakemake.output.duplicate_numts, index=False)
