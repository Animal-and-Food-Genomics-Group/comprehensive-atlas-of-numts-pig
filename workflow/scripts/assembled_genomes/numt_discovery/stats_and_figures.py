...
for index,row in genomes_info.iterrows():
    ...
    dropped[row['Genome Abbreviation']]=pd.read_csv(f"stats/numt_discovery/dropped_numts_close_to_extremity/{row['Species']}/{row['Assembly Name']}/{row['Assembly Accession']}_{row['Assembly Name']}.csv")
    dfs.append(tempdf)
df=pd.concat(dfs)
df=df.reset_index()
...
table1['dropped_numts_close_to_extremity']=table1.reset_index()['index'].apply(lambda x:len(dropped[x]['region_id'].unique())).tolist()
    ...
    for name2, group2 in group.groupby(by='region_id'):
        if len(group2)>1:
            if os.path.exists(f"{snakemake.params[0]}/{name}/{name2}.png"):
                ...
                custom_functions.plot_region(group2, f"{snakemake.params[0]}/{name}/{name2}.png")
        else:
            continue
...
for name, group in df.groupby(by='genome'):
    ...
    custom_functions.assign_range_and_plot(assembly, range=size_ranges, outcol=f'NUMT size range in {assembly_name} genome', outpath=f"{snakemake.params[1]}/{name}/numt_size_distribution_assembly.png")
    custom_functions.assign_range_and_plot(assembly, range=identity_ranges, outcol=f'NUMT sequence identity range in {assembly_name} genome', outpath=f"{snakemake.params[1]}/{name}/numt_identity_distribution_assembly.png")
...
plt.savefig(f"{snakemake.params[2]}/size_distribution.png", dpi=300, bbox_inches = "tight")
...
plt.savefig(f"{snakemake.params[2]}/identity_distribution.png", dpi=300, bbox_inches = "tight")
