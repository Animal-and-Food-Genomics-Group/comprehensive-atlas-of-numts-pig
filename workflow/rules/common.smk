...
mtc_genomes_for_dayama = pd.read_table(config["mtc_genomes_for_dayama"], header=None)[0].tolist()
...
try:
    all_regions, all_numts = glob_wildcards("data/phylogeny/sequences/all_sequences/{region}/{numt}.fasta")
except:
...
def get_chromosomes_from_report(species, name, accession):
    tempdf=pd.read_table(f"resources/genomes/{species}/{name}/ncbi/{accession}_{name}_assembly_report.tsv")
    chroms=tempdf['Sequence-ID'].tolist()
    return(chroms)

def get_chromosome_and_start_end_from_report(species, name, accession):
    tempdf=pd.read_table(f"resources/genomes/{species}/{name}/ncbi/{accession}_{name}_assembly_report.tsv")
    tempdict={chrom:end for chrom, end in zip(tempdf['Sequence-ID'], tempdf['Sequence-Length'])}
    return(tempdict)
